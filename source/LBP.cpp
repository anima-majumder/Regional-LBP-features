...
/************************************************************/
void LBP::CreateHistogram(uchar** ppArray, int algorithm, int type, char* const filename)
{	
	if(!ppArray)
	{
	...
	IplImage * lbp_img= NULL;
	lbp_img = cvCreateImage(cvSize(width, height), 8, 1);


	int bin;
	int x, y;
	std::ofstream fhist;
	//	char filename[] = "basic_histogram.txt";
	open2write(fhist, filename);
	...
	if(algorithm == 0) //LBP based histogram
	{
		...
		//allocate memory
		basic_bin.xy = (int)pow(2, SN.xy); // basic bin has size 2**8
		bin = basic_bin.xy;
		...
		for(y=R.yR; y<height-R.yR; y++){
			...
			for(x=R.xR; x<width-R.xR; x++){
				//std::cout << "y and x are "<<y << "\t" << x << std::endl;
				res_code = LBPCode(ppArray, algorithm, x, y); // LBP
									      // code
									      // for
									      // each
									      // pixel

				cvSetReal2D(lbp_img, y, x, res_code.xy);
				

			basic_hist.pHist_xy[res_code.xy]++; // histogram // at the level res_code.xy the points will get added
		...
		// replace the filename with extension jpg

		c.replace(c.find("txt"), sizeof("jpg")-1, "jpg");
		...
		std::strcpy(y, c.c_str());

		cvSaveImage(y, lbp_img);
		...
		//
		//		    std::cout<<"after "<<basic_hist.pHist_xy[i]<<std::endl;
		//		  }
...
//		for(size_t i=0; i<1; i++){
//
//				    std::cout<<"after norm "<<basic_hist.pHist_xy[i]<<std::endl;
//				  }
		//float sum_norm=0.0;
		//write the histogram
		/*
		for(size_t i=0; i<bin; i++)
		  {
		fhist<<basic_hist.pHist_xy[i] <<"\t";

		
		 // sum_norm = sum_norm + basic_hist.pHist_xy[i];
		  }
		*/

		//	std::cout<<"number of uni bin="<<uni_bin.xy<<std::endl;
		for(size_t i=0; i<uni_bin.xy+1; i++)
				  {
				    //  fhist<<uni_hist.pHist_xy[i] <<"\t"<<std::endl;
				    fhist<<uni_hist.pHist_xy[i] <<std::endl;
				  }
		//	fhist<<std::endl;
...
//	{
			for(y=R.yR; y<height-R.yR; y++)
			{
				for(x=R.xR; x<width-R.xR; x++)
				{				
					res_code = LBPCode(ppArray, algorithm, x, y, t);
...
//======================end of function to create histogram =========================//
...
/************************************************************/
struct Code LBP::LBPCode(uchar** ppArray, int algorithm, const int x, const int y)
{
	struct Code res_code={0,0,0};
	...
	if(algorithm==0 || algorithm==1)
	{
		...
		// here sample points implies neighbourhood points like 8, 16 etc
		for(i=0; i<number; i++)
		{
		...
		else	//with bilinear interpolation
		{
			for(i=0; i<number; i++)
			{
				...
				if(new_fp.x<1e-6 && new_fp.y<1e-6)  //interpolation is not needed
				  {
				    pData[i] = (uint)ppArray[(int)fp.y][(int)fp.x];
				  }
				else  //bilinear interpolation
				  {
				   

				    pData[i] = (uint)(ppArray[ltp.y][ltp.x]*(1-new_fp.x)*(1-new_fp.y)+
						      ppArray[rtp.y][rtp.x]*new_fp.x*(1-new_fp.y)+
						      ppArray[lbp.y][lbp.x]*(1-new_fp.x)*new_fp.y+
						      ppArray[rbp.y][rbp.x]*new_fp.x*new_fp.y);
				    //std::cout<<"bilinear interpolation over "<<std::endl;
				  }
			}
		}	
		...
		res_code.xy = 0;
		//binary to decimal conversion is done .. the second part gives binary value (either 0 or 1) and the first part does left shifting of the operand.
		//say (a << 1); Will return twice the value of a if a is an integer variable.
		for(i=number-1; i>=0; i--)
		  {

		    res_code.xy = (res_code.xy<<1) + (pData[i] >= (uint)ppArray[y][x]);  // check this :2* a +1 or 0 will give the decimal equivalent of the binary
...
//			for(i=number-1; i>=0; i--)
//			{
//				res_code.xt = (res_code.xt<<1) + (pData[i] >= (uint)pppArray[t][y][x]); // << is left shift (x << k == x multiplied by 2 to the power of k)
//			}
//		}
//
//		//y-t plane
...
void LBP::Create_block_wise_Histogram(uchar** ppArray,
				      int algorithm, int type, double* hist_data)
{	
  ...
  if(!ppArray)
	...
	if(algorithm == 0) //LBP based histogram
	{
		...
		cvSaveImage("lbp_img.jpg", lbp_img);
		...
		for(size_t i=0; i<uni_bin.xy+1; i++)
				  {
				    ...
				    hist_data[i] = uni_hist.pHist_xy[i];
